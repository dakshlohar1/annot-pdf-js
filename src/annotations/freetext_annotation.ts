  ...
  implements FreeTextAnnotation
{
  ...
  public writeAnnotationObject(cryptoInterface: CryptoInterface): number[] {
    ...
    ret = ret.concat(
      Util.convertNumberToCharArray(
        this.convertJustification(this.textJustification)
      )
    );
    ret.push(WriterUtil.SPACE);
  ...
  public validate(enact: boolean = true): ErrorList {
    ...
    if (
      this.calloutLine &&
      this.calloutLine.length > 0 &&
      this.freeTextType !== FreeTextType.FreeTextCallout
    ) {
      console.log(
        "Warning: Callout line only relevant for free text type: 'Callout'"
      );
    }

    if (this.fontSize < 0) {
      errorList.push(
        new InvalidFontSizeError("A font size < 0 is not allowed")
      );
    }
    ...
    if (typeof this.font === "string" || this.font instanceof Font) {
      ...
      if (!this.factory.parser.getFonts().isRegisteredFont(this.font)) {
        errorList.push(
          new InvalidFontError(
            "Only fonts registered in the PDF and standard fonts are allowed"
          )
        );
      }
    } else {
