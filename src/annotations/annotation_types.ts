...
import {
  ...
  InvalidRectError,
  InvalidDateError,
  InvalidReferencePointerError,
  ColorOutOfRangeError,
  InvalidColorError,
  InvalidIDError,
...
export type ICTM = [number, number, number, number, number, number];
...
export class BaseAnnotationObj implements BaseAnnotation {
  ...
  public writeAnnotationObject(cryptoInterface: CryptoInterface): number[] {
    ...
    ret.push(WriterUtil.SPACE);
    ret = ret.concat(
      WriterUtil.writeReferencePointer(this.pageReference!.object_id!, true)
    );
    ret.push(WriterUtil.SPACE);
    ...
    ret = ret.concat(
      Array.from(
        Util.escapeString(
          cryptoInterface.encrypt(
            new Uint8Array(Util.convertStringToAscii(this.id)),
            this.object_id
          )
        )
      )
    ...
    ret = ret.concat(
      Array.from(
        Util.escapeString(
          cryptoInterface.encrypt(
            new Uint8Array(
              Util.convertStringToAscii(this.updateDate as string)
            ),
            this.object_id
          )
        )
    ...
    if (this.border) {
      ...
      ret = ret.concat(
        WriterUtil.writeNumberArray([
          this.border.horizontal_corner_radius || 0,
          this.border.vertical_corner_radius || 0,
    ...
    if (this.color && this.shouldWriteColor) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        WriterUtil.writeNumberArray([this.color.r, this.color.g, this.color.b])
      );
      ret.push(WriterUtil.SPACE);
    }
  ...
  public validate(enact: boolean = true): ErrorList {
    ...
    if (this.takeAppearanceStreamFrom) {
      if (typeof this.takeAppearanceStreamFrom === "string") {
        // lookup appearance stream
        let res: Annotation[] = [];
        this.factory._getAnnotations().forEach((annots: any) => {
          res = annots.filter(
            (value: any) => value.id === this.takeAppearanceStreamFrom
          );
        });

        if (res.length === 0 || res.length > 1) {
          errorList.push(
            new InvalidAnnotationReference(
              "The provided string referencing the annotation to take the appearance stream from is not valid."
            )
          );
        }
        ...
        if (!res[0].appearanceStream) {
          errorList.push(
            new InvalidAppearanceStreamError(
              "The referenced annotation has no specified appearance stream."
            )
          );
        }
  ...
  protected checkReferencePointer(
    ptr: ReferencePointer | undefined
  ): ErrorList {
    let errorList: ErrorList = [];
    if (
      !(
      ...
      errorList.push(
        new InvalidReferencePointerError("Invalid reference pointer")
      );
    }
  ...
  protected checkRect(nr: number, rect: number[]): ErrorList {
    ...
    if (rect.length !== nr) {
      errorList.push(
        new InvalidRectError(
          "Rect has invalid number of entries: " +
            rect +
            " has " +
            rect.length +
    ...
    rect.forEach((a) => {
      if ("number" !== typeof a) {
        errorList.push(
          new InvalidRectError("Rect " + rect + " has invalid entry: " + a)
        );
      }
    });
  ...
  public extract(annot_obj: any, page: any, cryptoInterface: CryptoInterface) {
    ...
    if (annot_obj["/M"])
      this.updateDate = Util.convertUnicodeToString(
        cryptoInterface.decrypt(annot_obj["/M"], this.object_id)
      );

    if (annot_obj["/Border"]) this.border = annot_obj["/Border"];

    if (annot_obj["/C"]) this.color = annot_obj["/C"];

    if (annot_obj["/NM"])
      this.id = Util.convertUnicodeToString(
        cryptoInterface.decrypt(annot_obj["/NM"], this.object_id)
      );

    if (annot_obj["/Contents"])
      this.contents = Util.convertUnicodeToString(
        cryptoInterface.decrypt(annot_obj["/Contents"], this.object_id)
      );

    if (annot_obj["/AP"])
      this.appearanceStream = AppearanceStreamParser.parse(
        this,
        annot_obj["/AP"]
      );
  }
...
export class MarkupAnnotationObj
  extends BaseAnnotationObj
  implements MarkupAnnotation
{
  author: string = "";
  opacity?: number = 1; // /CA
  creationDate?: string | Date; // /CreationDate
  ...
  public writeAnnotationObject(cryptoInterface: CryptoInterface): number[] {
    ...
    ret = ret.concat(
      Array.from(
        Util.escapeString(
          cryptoInterface.encrypt(
            new Uint8Array(Util.convertStringToAscii(this.author)),
            this.object_id
          )
        )
      )
      ...
      ret = ret.concat(
        Array.from(
          Util.escapeString(
            cryptoInterface.encrypt(
              new Uint8Array(
                Util.convertStringToAscii(this.creationDate as string)
              ),
              this.object_id
            )
          )
    ...
    if (this.subject !== "") {
      ...
      ret = ret.concat(
        Array.from(
          Util.escapeString(
            cryptoInterface.encrypt(
              new Uint8Array(Util.convertStringToAscii(this.subject)),
              this.object_id
            )
          )
        )
    ...
    if (this.richtextString) {
      ...
      ret = ret.concat(
        Array.from(
          Util.escapeString(
            cryptoInterface.encrypt(
              new Uint8Array(Util.convertStringToAscii(this.richtextString!)),
              this.object_id
            )
          )
        )
  ...
  public extract(annot_obj: any, page: any, cryptoInterface: CryptoInterface) {
    ...
    if (annot_obj["/T"])
      this.author = Util.convertUnicodeToString(
        cryptoInterface.decrypt(annot_obj["/T"], this.object_id)
      );
  }
}
