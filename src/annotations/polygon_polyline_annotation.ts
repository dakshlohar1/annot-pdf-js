...
export class PolygonPolyLineAnnotationObj
  extends MarkupAnnotationObj
  implements PolygonPolyLineAnnotation
{
  ...
  public validate(enact: boolean = true): ErrorList {
    ...
    if (this.vertices.length % 2 !== 0) {
      errorList.push(
        new InvalidVerticesError("number of vertices must be an even number")
      );
    }
...
export class PolyLineAnnotationObj
  extends PolygonPolyLineAnnotationObj
  implements PolyLineAnnotation
{
  ...
  public createDefaultAppearanceStream() {
    ...
    go.setLineColor(this.color)
      ...
      .drawPolygon(this.vertices, this.border?.border_width || 1);
...
export class PolygonAnnotationObj
  extends PolygonPolyLineAnnotationObj
  implements PolygonAnnotation
{
  constructor() {
    super();
    this.type = "/Polygon";
