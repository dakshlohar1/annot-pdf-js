...
import {
  ...
  AnnotationIcon,
  TextAnnotationObj,
} from "./annotations/text_annotation";
...
export class Writer {
  ...
  sortPageWise(annotations: Annotation[]): { [item: number]: Annotation[] } {
    ...
    for (let annot of annotations) {
      if (!pageAnnots[annot.page]) pageAnnots[annot.page] = [];
  ...
  adaptPageObject(
    ...
    if (!page.object_id) throw Error("Page without object id");
    ...
    if (page_ptr.compressed) {
      ...
      let obj_data = obj.stream
        .getData()
        .slice(obj.pointer_stream_start, obj.pointer_stream_end + 1);

      let ref_ptr = WriterUtil.writeReferencePointer(obj.id, false).concat(32);

      let new_data: Uint8Array = new Uint8Array(
        ref_ptr.length +
          WriterUtil.OBJ.length +
          obj_data.length +
          WriterUtil.ENDOBJ.length
      );
      ...
      new_data.set(
        WriterUtil.ENDOBJ,
        WriterUtil.OBJ.length + obj_data.length + ref_ptr.length
      );

      return WriterUtil.replaceAnnotsFieldInPageObject(
        new_data,
        page,
        0,
  ...
  writeAnnotArray(
    ...
    if (!page) throw Error("Missing page reference");
    ...
    references = references.concat(
      annots.map((x) => {
        if (!x.object_id) throw Error("Annotation with object_id null");
    ...
    references = references.filter((a: any) => {
      let toDel = toDelete.find(
        (t) =>
          (<any>t.object_id).obj === a.obj &&
          (<any>t.object_id).generation === a.generation
      );
  ...
  writeCrossSiteReferenceTable(): number[] {
    ...
    for (let sequence of ordered_sequences) {
      ...
      for (let i = 0; i < sequence.length; ++i) {
        ...
        if (sequence[i].free) _ret.push(WriterUtil.F);

        if (sequence[i].update) _ret.push(WriterUtil.N);

        _ret.push(WriterUtil.CR);
        _ret.push(WriterUtil.LF);

        if (_ret.length < 20) throw Error("XRef entry < 20 bytes");
  ...
  writeTrailer(posXref: number): number[] {
    ...
    ret.push(WriterUtil.SPACE);
    ret = ret.concat(
      Util.convertNumberToCharArray(this.parser.documentHistory.trailerSize)
    );
    ret.push(WriterUtil.SPACE);
    ...
    if (trailer.id) {
      ...
      for (let val of trailer.id) {
        ...
        ret = ret.concat(
          Util.convertStringToAscii(Util.convertByteArrayToHexString(val))
        );
    ...
    if (!trailer.root)
      throw Error(
        "No root object in trailer, although this is an cross reference table document"
      );
    ...
    ret.push(WriterUtil.SPACE);
    ret = ret.concat(
      Util.convertNumberToCharArray(
        this.parser.documentHistory.getRecentUpdate().start_pointer
      )
    );
    ret.push(WriterUtil.SPACE);
  ...
  /**
   * Writes the annotations at the end of the PDF byte representation and returns
   * the byte array
   * */
  write(): Uint8Array {
    ...
    // Fix case that there is no line-break after the end of the file
    ...
    for (let key in pageWiseSortedToDelete) {
      let del_data = this.updatePageAnnotationReferenceArray(
        pageWiseSortedToDelete[key]
      );
      this.xrefs.push({
        id: del_data.ptr.obj,
        pointer: ptr,
    ...
    for (let font of fonts) {
      if (!font.object_id) throw Error("Font has no object id");
