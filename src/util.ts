...
import { Color, ICTM } from "./annotations/annotation_types";
...
export class Util {
  ...
  public static rotate(
    angle: number,
    ctm: ICTM,
    options: { origin: number[] | null } = { origin: null }
  ): ICTM {
    let y;
    if (angle === 0) return ctm;
    ...
    return Util.getTransformed(cos, sin, -sin, cos, x, y, ctm);
  }

  public static getTransformed(
    m11: number,
    m12: number,
    m21: number,
    m22: number,
    ...
    return [m11, m12, m21, m22, dx, dy].map((v) => Number(v)) as ICTM;
  }

  /**
   * Extracts the version information of a PDF file
   * */
  public static extractVersion(
    data: Uint8Array,
    index: number = 0
  ): PDFVersion {
    let ptr_version_start =
      Util.locateSequence(Util.VERSION, data, index) + Util.VERSION.length;
    let ptr_delimiter = Util.locateSequence(
      [Util.DOT],
      data,
      ptr_version_start
    );
    let major_version = Util.extractNumber(
      data,
      ptr_version_start,
      ptr_delimiter
    ).result;
    let ptr_end = Util.locateDelimiter(data, ptr_delimiter);
    let minor_version = Util.extractNumber(
      data,
      ptr_delimiter + 1,
      ptr_end
    ).result;
  ...
  public static readNextWord(
    ...
    if (index <= data.length)
      return {
        result: data.slice(start_index, index),
   ...
   * Skips only spaces
   * */
  public static skipSpacesReverse(
    data: Uint8Array | number[],
    index: number = 0
  ): number {
    while (
   ...
   * another delimiter or reaches the end of the document
   * */
  public static skipDelimiterReverse(
    data: Uint8Array,
    index: number = 0
  ): number {
  ...
  public static isSpace(value: number): boolean {
    return value === Util.LF || value === Util.CR || value === Util.SPACE;
  }
   ...
   * */
  public static locateSequenceReversed(
    sequence: number[],
    data: Uint8Array,
    offset: number = data.length - 1,
    closed: boolean = false
  ): number {
    let i = offset;
   ...
   * or a space (32)
   * */
  public static locateDelimiterReversed(
    data: Uint8Array,
    offset: number = data.length - 1
  ): number {
  ...
  public static extractReference(data: Uint8Array, index: number): Uint8Array {
    index = Util.skipDelimiter(data, index);
    let r_index = this.locateSequence(
      this.convertStringToAscii(" R"),
      data,
      index,
      true
    );
   ...
   * */
  public static extractReferenceTyped(
    data: Uint8Array,
    index: number
  ): ExtractionResult {
    ...
    return {
      result: { obj: id, generation: gen },
      start_index: index,
      end_index: index + ref_data.length + 1,
    }; // + _R
  }
   ...
   * */
  public static extractHexString(
    data: Uint8Array,
    index: number
  ): ExtractionResult {
    ...
    return {
      result: new Uint8Array(Util.convertHexStringToByteArray(data)),
  ...
  public static extractString(
    ...
    let string_start = Util.locateSequence(
      Util.LITERAL_STRING_START,
      data,
      index
    );
    let string_end = Util.locateSequence(Util.LITERAL_STRING_END, data, index);

    while (data[string_end - 1] === 92) {
      // "\" escape
      string_end = Util.locateSequence(
        Util.LITERAL_STRING_END,
        data,
        string_end + 1
      );
    }
    ...
    return {
      result: Util.unescapeString(data),
      start_index: string_start,
      end_index: string_end,
    };
  }
   ...
   * */
  public static extractOptionValue(
    data: Uint8Array,
    index: number = 0
  ): ExtractionResult {
    if (data[index] !== 47) throw Error("misplaced option value pointer");

    let end = Util.locateDelimiter(data, index + 1);

    return {
      result: Util.convertAsciiToString(
        Array.from(data.slice(index + 1, end + 1))
      ),
      start_index: index,
      end_index: end,
    };
      ...
      let uintToString = (uintArray: any) => {
        ...
        for (let i = 0; i < uintArray.length - 1; i += 2) {
          ret += String.fromCharCode(
            (uintArray[i] << 8) | (uintArray[i + 1] & 0xff)
          );
        }
   ...
   * */
  public static convertHexStringToByteArray(
    hex_string: string | Uint8Array | number[]
  ): number[] {
    let ret_val: number[] = [];
    ...
    for (let i = 0; i < hex_string.length - 1; i += 2) {
      ret_val.push(
        (parseInt(hex_string.charAt(i), 16) << 4) +
          parseInt(hex_string.charAt(i + 1), 16)
      );
    }
   ...
   * */
  public static convertByteArrayToHexString(
   ...
   * */
  public static areArraysEqual(
    array_one: Uint8Array | number[],
    array_two: Uint8Array | number[]
  ): boolean {
  ...
  public static debug_printIndexed(array: Uint8Array | number[]) {
    ...
    for (let i = 0; i < array.length; ++i) {
      ...
      if (Util.isSpace(array[i])) outp += " .";
      else outp += " " + String.fromCharCode(array[i]);
    }
   ...
   * */
  public static convertUint8ArrayToInt32Array(
    a: Uint8Array | number[]
  ): Int32Array {
   ...
   * */
  public static convertInt32ArrayToUint8Array(
    a: Int32Array | number[]
  ): Uint8Array {
  ...
  public static convertStringToByteString(value: string): number[] {
    ...
    let hex_val: string = Util.convertByteArrayToHexString(
      Util.convertStringToAscii(value)
    );
