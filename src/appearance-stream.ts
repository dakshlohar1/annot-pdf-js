...
export class AppStream implements AppearanceStream {
  ...
  lookupNContentStream() {
    if (!this.N) {
      console.warn(
        "call lookupNContentStream without set content stream value"
      );
      return;
    } else if (Util.isReferencePointer(this.N)) {
      this.N = this.annot.factory.parser.extractXObject(
        this.N as ReferencePointer
      );
  ...
  private writeAppearanceStreamObj(
    ap: XObject | OnOffAppearanceStream | ReferencePointer
  ): number[] {
    let ret: number[] = [];

    if (Util.isReferencePointer(ap)) {
      ret = ret.concat(
        WriterUtil.writeReferencePointer(ap as ReferencePointer, true)
      );
      ...
      ret = ret.concat(
        WriterUtil.writeReferencePointer((ap as XObjectObj).object_id!, true)
      );
    } else {
...
export class XObjectObj implements XObject {
  ...
  public writeXObject(
    ...
    if (!this.object_id) throw Error("object_id of XObject not set");
    ...
    let stream_data: number[] = this.contentStream
      ? this.contentStream.writeContentStream(true)
      : [];
    return WriterUtil.writeStreamObject(
      this.object_id,
      ret,
      shouldCompress
        ? new FlateStream(
            new Uint8Array(stream_data),
            undefined,
            true,
            cryptoInterface,
            this.object_id
