...
export class Operator {
  ...
  constructor(
    public name: string = "",
    public parameters: (number | string | ReferencePointer)[] = []
  ) {}
...
export class GraphicsObject extends Operator {
  drawFillRect(
    ...
    if (
      cornerRadius &&
      (2 * cornerRadius > Math.abs(x_1 - x_2) ||
        2 * cornerRadius > Math.abs(y_1 - y_2))
    ) {
      throw Error(
        "Invalid corner radius. Must be small than two times the linewidth"
      );
    }
...
export class TextObject extends Operator {
  ...
  setTextRelative(
    text: string,
    x_rel: number | number[] | undefined = undefined,
    y_rel: number | undefined = undefined
  ) {
    if (text === "") return this;
  ...
  formatText(
    ...
    // just place the text
    if (rect_width < TextObject.SINGLE_CHAR_WIDTH) {
      // if provided rectangle is too small to present a single letter just write it but warn
      console.warn(
        `Overfull hbox ${rect_width} is below minimal threshold of ${TextObject.SINGLE_CHAR_WIDTH}`
      );
      this.setText(text, [
        calc_just(font.calculateTextDimensions(text, textSize)[0]),
