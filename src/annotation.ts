...
import {
  HighlightAnnotationObj,
  UnderlineAnnotationObj,
  SquigglyAnnotationObj,
  StrikeOutAnnotationObj,
} from "./annotations/text_markup_annotation";
import { FreeTextAnnotationObj } from "./annotations/freetext_annotation";
import {
  SquareAnnotationObj,
  CircleAnnotationObj,
} from "./annotations/circle_square_annotation";
import {
  ...
  static parseParameters(values: any[]) {
    ...
    if (typeof values[i] === "number") {
      ...
      if ("r" in values[i] && "g" in values[i] && "b" in values[i]) {
        ...
        values[i].length > 0 &&
        typeof values[i][0] === "number"
      ) {
        ret_val.vertices = values[i++];
      } else if (
        Array.isArray(values[i]) &&
        values[i].length > 0 &&
        Array.isArray(values[i][0])
      ) {
        ret_val.inkList = values[i++];
      } else {
        throw Error(
          "Invalid parameter provided - is neither color, nor quadpoints array or an inklist"
        );
      }
...
export class AnnotationFactory {
  ...
  private generateUniqueIdentifier() {
    return (
      "pdfAnnotate-" +
      Util.convertDateToPDFDate(new Date()).slice(3, 17) +
      "-" +
  ...
  createLineAnnotation(...values: any[]): LineAnnotationObj {
    let params = ParameterParser.parseParameters(values);

    let annot: LineAnnotationObj = new LineAnnotationObj();
    annot = (<any>Object).assign(annot, this.createBaseAnnotation(params.page));
    annot = (<any>Object).assign(annot, params);
    annot.validate();
    this.annotations.push(annot);
  ...
  createStampAnnotation(...values: any[]) {
    let params = ParameterParser.parseParameters(values);
    let annot: Annotation = (<any>Object).assign(
      this.createBaseAnnotation(params.page),
      {
        opacity: 1,
        initiallyOpen: false,
        annotation_flag: 4,
      ...
      for (let i = 0; i < this.annotations.length; ++i) {
        if ("string" === typeof id && this.annotations[i].id === id) {
          this.annotations = this.annotations = [
            ...this.annotations.slice(0, i),
            ...this.annotations.slice(i + 1),
          ];
          resolve(this.toDelete);
          return;
        } else if (
          id.obj &&
          this.annotations[i].object_id &&
          id.obj === (<any>this.annotations[i].object_id).obj &&
          id.generation &&
          id.generation === (<any>this.annotations[i].object_id).generation
        ) {
          this.annotations = this.annotations = [
            ...this.annotations.slice(0, i),
            ...this.annotations.slice(i + 1),
        ...
        for (let _annots of annots) {
          for (let annot of _annots) {
            if ("string" === typeof id && annot.id === id) {
              ...
              id.obj &&
              annot.object_id &&
              id.obj === annot.object_id.obj &&
              id.generation &&
              id.generation === annot.object_id.generation
            ) {
              this.toDelete.push(annot);
              resolve(this.toDelete);
              return;
