...
import { ITextMargin } from "./annotations/freetext_annotation";
...
export class Font {
  ...
  public proposeLinebreaks(
    ...
    // Adjust the available width by subtracting left and right margins
    let availableWidth = width - (textMargin.left + textMargin.right);
    ...
    for (let l = 0; l < ascii.length; ++l) {
      ...
      if (line_width + ascii[l] > availableWidth) {
        // backtrack to last space in the same line and move remainder in the next line
        if (last_space !== -1) {
          positions.push({
            start: last_pos,
            end: last_space - 1,
            width: ascii.slice(last_pos, last_space).reduce((x, y) => x + y, 0),
          });
          l = last_space + 1;
          last_pos = l;
          last_space = -1;
        } else {
          // if no such last space is in the line return line_width - last letter width
          positions.push({
            start: last_pos,
            end: l - 1,
            width: ascii.slice(last_pos, l).reduce((x, y) => x + y, 0),
          });
          last_pos = l;
        }
        line_width = 0;
    ...
    if (last_pos !== ascii.length) {
      positions.push({
        ...
        width: ascii.slice(last_pos, ascii.length).reduce((x, y) => x + y, 0),
      });
    }
  ...
  public getTextWidthArray(text: string, fontSize: number): number[] {
    ...
    for (let letter of ascii) {
      ...
      ret_val.push(
        ((this.widths[letter - this.firstChar] + kerning_value) / 1000) *
          fontSize
      );
  ...
  private populateStandardFontData(font_name: string) {
    ...
    let key = Object.keys(STANDARD_FONT_DATA).filter(
      (name) => font_name.localeCompare(name) === 0
    );
    ...
    let font_data =
      STANDARD_FONT_DATA[key[0] as keyof typeof STANDARD_FONT_DATA];
   ...
   * */
  public static createStandardFont(
  ...
  public writeFontDescriptor(): number[] {
    ...
    if (this.fontDescriptor.fontFamily) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        Util.convertStringToByteString(this.fontDescriptor.fontFamily)
      );
      ret.push(WriterUtil.SPACE);
    }
    ...
    if (this.fontDescriptor.fontStretch) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        Util.convertStringToAscii(this.fontDescriptor.fontStretch)
      );
      ret.push(WriterUtil.SPACE);
    }
    ...
    if (this.fontDescriptor.flags) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        Util.convertNumberToCharArray(this.fontDescriptor.flags)
      );
      ret.push(WriterUtil.SPACE);
    }
    ...
    if (this.fontDescriptor.fontBBox) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        WriterUtil.writeNumberArray(this.fontDescriptor.fontBBox)
      );
      ret.push(WriterUtil.SPACE);
    }
    ...
    if (this.fontDescriptor.italicAngle) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        Util.convertNumberToCharArray(this.fontDescriptor.italicAngle)
      );
      ret.push(WriterUtil.SPACE);
    }
    ...
    if (this.fontDescriptor.ascent) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        Util.convertNumberToCharArray(this.fontDescriptor.ascent)
      );
      ret.push(WriterUtil.SPACE);
    }
    ...
    if (this.fontDescriptor.descent) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        Util.convertNumberToCharArray(this.fontDescriptor.descent)
      );
      ret.push(WriterUtil.SPACE);
    }
    ...
    if (this.fontDescriptor.leading) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        Util.convertNumberToCharArray(this.fontDescriptor.leading)
      );
      ret.push(WriterUtil.SPACE);
    }
    ...
    if (this.fontDescriptor.capHeight) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        Util.convertNumberToCharArray(this.fontDescriptor.capHeight)
      );
      ret.push(WriterUtil.SPACE);
    }
    ...
    if (this.fontDescriptor.xHeight) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        Util.convertNumberToCharArray(this.fontDescriptor.xHeight)
      );
      ret.push(WriterUtil.SPACE);
    }
    ...
    if (this.fontDescriptor.stemV) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        Util.convertNumberToCharArray(this.fontDescriptor.stemV)
      );
      ret.push(WriterUtil.SPACE);
    }
    ...
    if (this.fontDescriptor.stemH) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        Util.convertNumberToCharArray(this.fontDescriptor.stemH)
      );
      ret.push(WriterUtil.SPACE);
    }
    ...
    if (this.fontDescriptor.avgWidth) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        Util.convertNumberToCharArray(this.fontDescriptor.avgWidth)
      );
      ret.push(WriterUtil.SPACE);
    }
    ...
    if (this.fontDescriptor.maxWidth) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        Util.convertNumberToCharArray(this.fontDescriptor.maxWidth)
      );
      ret.push(WriterUtil.SPACE);
    }
    ...
    if (this.fontDescriptor.missingWidth) {
      ...
      ret.push(WriterUtil.SPACE);
      ret = ret.concat(
        Util.convertNumberToCharArray(this.fontDescriptor.missingWidth)
      );
      ret.push(WriterUtil.SPACE);
    }
  ...
  public writeFont(): number[] {
    if (!this.object_id) throw Error("object_id of font not set");
...
export class FontManager {
  ...
  public getFont(font: ReferencePointer | Font | string): Font | undefined {
    for (let f of this.fonts) {
      if (typeof font === "string") {
        if (f.name === font || f.baseFont === font) return f;
      } else if (font instanceof Font) {
        if (font.object_id && f.object_id) {
          if (
            f.object_id.obj === font.object_id.obj &&
            f.object_id.generation === font.object_id.generation
          )
            return f;
        } else if (font.name && f.name) {
          if (f.name === font.name) return f;
        } else if (font.baseFont && f.baseFont) {
          if (f.baseFont === font.baseFont) return f;
        }
      } else {
        if (
          f.object_id!.obj === (font as ReferencePointer).obj &&
          f.object_id!.generation === (font as ReferencePointer).generation
        )
          return f;
      }
    }
    ...
    if (typeof font === "string") {
      if (Font.isStandardFont(font)) {
        font = Font.createStandardFont(
          this.parser.getFreeObjectId(),
          this.getUnusedFontName(),
          font
        );
        font.is_new = true;
      }
  ...
  public hasFont(font_ptr: Font | ReferencePointer | string): boolean {
    if (font_ptr instanceof Font && font_ptr.object_id) {
      ...
      return (
        this.fonts.filter((f) => f.name === font_ptr || f.baseFont === font_ptr)
          .length > 0
      );
    }
    ...
    return (
      this.fonts.filter(
        (f) =>
          f.object_id &&
          f.object_id.obj === (font_ptr as ReferencePointer).obj &&
          f.object_id.generation === (font_ptr as ReferencePointer).generation
      ).length > 0
    );
  }
  ...
  public isRegisteredFont(font: string | Font): boolean {
    if (typeof font === "string") {
      if (Font.isStandardFont(font)) return true;
